...
def test_sacn_engine_dmx_state_integration():
    """Toggle mode: fire and kill both pulse same channel for PULSE_FRAMES, consumed by engine."""
    P = PULSE_FRAMES
    dmx = DmxState(cue_channel_map={41: [41]})

    # Fire pulse without engine — P frames of 255
    dmx.fire(41)
    ...
    # Kill pulse without engine — same channel, P frames
    dmx.kill(41)
    assert dmx.get_channel(41) == 255
    for _ in range(P):
        frame = dmx.snapshot()
...
def test_sacn_with_cue_output_adapter():
    ...
    # Kill pulse — same channel, P frames
