"""
VisionDJTab V9 - Tab for DJ Detector with YOLO ROI Detection
Preview + LayeredZoneEditor PRO + Debug Overlay + TEST FIRE buttons

Phase 9.0: YOLO-based ROI-only detection, multi-zone support
...
class VisionDJTab(QWidget):
    ...
    def __init__(self, vision_manager, parent=None):
        ...
        try:
            self.vision_manager.set_ui_callback_dj(self._on_frame_received)
        except AttributeError:
            print("[VisionDJTab] Warning: set_ui_callback_dj() not available")

        # Load existing zones
        self._load_zones()

        print("[VisionDJTab] Initialized with thread-safe frame buffer")
    ...
    def _build_preview_panel(self) -> QGroupBox:
        ...
        group = QGroupBox("Vista Previa - Cámara DJ + YOLO ROI Detection")
        layout = QVBoxLayout(group)

        # LayeredZoneEditor PRO
        self.zone_editor = LayeredZoneEditor(zone_list=[], max_zones=5, camera_type="DJ")
    ...
    def _build_controls_panel(self) -> QGroupBox:
        ...
        group = QGroupBox("DJ Detector V9 - YOLO")
        layout = QVBoxLayout(group)

        # Enable toggle
        self.dj_enabled_check = QCheckBox("Habilitado")
        self.dj_enabled_check.setChecked(False)
        self.dj_enabled_check.stateChanged.connect(self._on_enabled_changed)
        ...
        for i in range(5):
            ...
            # LED
            led = LEDIndicator(QColor(0, 255, 0), 14)
            self.led_dj_cues.append(led)
            cues_layout.addWidget(led, row, 0)

            # Label
            cues_layout.addWidget(QLabel(f"DJ_{zone_id} (C{cue_id})"), row, 1)
    ...
    def _on_frame_received(self, frame):
        """
        Callback para recibir frames de la cámara DJ.
    ...
    def _update_preview(self):
        ...
        try:
            ...
            print(f"[VisionDJTab] Error updating frame: {e}")

    def _draw_debug_overlay(self, frame):
        """Draw debug overlay on frame: ROI rects, zone info."""
        try:
            dj_detector = self.vision_manager.get_dj_detector()
            if not dj_detector:
                return frame

            state = dj_detector.get_state()
            ...
            for zone in zones:
                ...
                # Color: green if active, yellow if detected, red if not visible, gray otherwise
                if not visible:
                ...
                text = f"Z{zone_id} {status_str} {conf:.2f}"
            ...
            metrics = dj_detector.get_metrics()
            ...
            print(f"[VisionDJTab] Debug overlay error: {e}")
    ...
    def _update_state(self):
        ...
        try:
            ...
            dj_detector = self.vision_manager.get_dj_detector()
            if dj_detector:
                detector_state = dj_detector.get_state()
                metrics = dj_detector.get_metrics()
                ...
                for i in range(5):
                    zone_id = i + 1
                    self.led_dj_cues[i].set_on(zone_id in active_zones)

                # Update debug list
                self._update_zone_debug_list(detector_state)

                # Update metrics label
                health = dj_detector.get_health()
            ...
            self.dj_enabled_check.blockSignals(True)
            self.dj_enabled_check.setChecked(dj_state.get("enabled", False))
            self.dj_enabled_check.blockSignals(False)

        except Exception as e:
            print(f"[VisionDJTab] Error updating state: {e}")
    ...
    def _load_zones(self):
        ...
        try:
            ...
            zones = config.get_dj_zones()
            self.zone_editor.set_zones(zones)
        except Exception as e:
            print(f"[VisionDJTab] Error loading zones: {e}")
    ...
    def _on_zone_visible_changed(self, zone_id: int, state: int):
        ...
        try:
            dj_detector = self.vision_manager.get_dj_detector()
            if dj_detector:
                dj_detector.set_zone_visible(zone_id, visible)
                print(f"[VisionDJTab] Zone {zone_id} visible={visible}")
        except Exception as e:
            print(f"[VisionDJTab] Error setting zone visibility: {e}")
    ...
    def _on_test_fire(self, zone_id: int):
        ...
        try:
            dj_detector = self.vision_manager.get_dj_detector()
            if dj_detector:
                result = dj_detector._test_fire_zone(zone_id)
                print(f"[VisionDJTab] TEST FIRE Zone {zone_id}: {result}")
        except Exception as e:
            print(f"[VisionDJTab] Error test firing: {e}")
    ...
    def _on_kill_all(self):
        ...
        try:
            dj_detector = self.vision_manager.get_dj_detector()
            if dj_detector:
                result = dj_detector._test_fire_zone(1, force_off=True)
                print(f"[VisionDJTab] KILL ALL: {result}")
        except Exception as e:
            print(f"[VisionDJTab] Error killing all: {e}")
    ...
    def _on_apply(self):
        ...
        try:
            # Update zones in DJDetector
            zones = self.zone_editor.get_zones()
            self.vision_manager.set_dj_zones(zones)

            # Update delay
            dj_detector = self.vision_manager.get_dj_detector()
            if dj_detector:
                dj_detector.set_disappear_delay(self.delay_spin.value())

            # Save to config JSON
            config = self.vision_manager.get_config()
            dj_config = config.get_dj_config()
            dj_config["zones"] = zones
            dj_config["zones_count"] = len(zones)
            dj_config["disappear_delay"] = self.delay_spin.value()

            config.set("dj", dj_config)
            config.save()

            print(f"[VisionDJTab] Configuration applied ({len(zones)} zones)")

        except Exception as e:
            print(f"[VisionDJTab] Error applying config: {e}")
